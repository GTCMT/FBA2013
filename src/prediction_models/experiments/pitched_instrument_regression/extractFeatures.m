...
%     features(1,12)=max(timbreMeasure(2,:))-min(timbreMeasure(2,:));
    ...
    features(1,14)=mean(amp_hist_feature);
    features(1,15)=std(amp_hist_feature);
    features(1,16)=max(amp_hist_feature);
    features(1,17)=min(amp_hist_feature);
...
%     features(1,22)=max(ampenv_peaks)-min(ampenv_peaks);
    ...
    features(1,22) = numGoodNotes(note,thresh1,thresh2);
